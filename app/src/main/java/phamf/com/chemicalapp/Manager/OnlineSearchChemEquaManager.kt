...
typealias OnReachASearch = (list : ArrayList<RO_ChemicalEquation>) -> Unit
...
class OnlineSearchChemEquaManager (
    ...
    fun search (link : String, page_num : Int = 1) {
        ...
        var observer = Observable.just(link)
                ...
                .map {
                    hyper_link ->
                    var list = ArrayList<RO_ChemicalEquation>()
                    if (hyper_link != "") {
                        var doc = Jsoup.connect(hyper_link).get()
                        Log.e("Link", hyper_link + "")
                        ...
                        for (i in 0..elements.size - 1) {
                            var equation = RO_ChemicalEquation()
                            list.add(equation)
                        }
