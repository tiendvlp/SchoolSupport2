 ...
 */
class GraphDrawerActivity : FullScreenActivity() {

    lateinit var viewModel : MVVM_GraphDrawerActivityPresenter
    ...
    fun setUpViewModel () {
        ...
        viewModel = MVVM_GraphDrawerActivityPresenter(application)
        ...
        graph_btn_setting.setOnClickListener {
            graph_setting_form.visibility = if (graph_setting_form.visibility == View.VISIBLE) {
                ...
                if ((start_x_val != graph.start_X_value) or (end_x_val != graph.end_X_value) or (zoom_index != graph.zoom_index)) {
                    ...
                    graph.zoom_index = 50 + zoom_index * 10
                    graph.draw()
